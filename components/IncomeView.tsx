    ...
    <div className="space-y-6">
      {isConverting && (
        <div className="bg-sky-900/50 border border-sky-700 text-sky-300 px-4 py-3 rounded-lg flex items-center" role="alert">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mr-3 flex-shrink-0" fill="none" viewBox="0 0 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <span className="block sm:inline">
            {t('displayingIn', { code: displayCurrency.code, rate: conversionRate.toFixed(2), baseCode: baseCurrency.code })}
          </span>
        </div>
      )}
      ...
      <div>
        ...
        <form onSubmit={handleAddSource} className="glass-card bg-opacity-30 p-4 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end">
            <div><label htmlFor="source-amount" className="block text-sm font-medium text-slate-300 mb-1">{t('amount')} ({baseCurrency.code})</label><div className={`flex items-center bg-slate-700/50 border rounded-lg focus-within:ring-2 transition-all ${errors.amount ? 'border-red-500 focus-within:ring-red-500' : 'border-slate-600 focus-within:ring-sky-500'}`}><span className="py-2 pl-3 pr-2 text-slate-400 border-r border-slate-600">{baseCurrency.symbol}</span><input id="source-amount" name="amount" type="number" value={sourceAmount} onChange={(e) => { setSourceAmount(e.target.value); if (errors.amount) setErrors(p => ({ ...p, amount: undefined })); }} placeholder={t('amount')} min="0.01" step="0.01" className="flex-grow bg-transparent text-white py-2 px-2 outline-none" aria-label={t('incomeSourceAmount')} aria-invalid={!!errors.amount} /></div><p className="text-red-400 text-sm mt-1">{errors.amount}</p></div>

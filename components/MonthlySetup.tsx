...
export const MonthlySetup: React.FC<MonthlySetupProps> = ({ onSetup, onCancel, existingMonths, isSubmitting, submissionError }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (existingMonths.includes(monthId)) {
        setError(`A budget for ${monthId} already exists.`);
        return;
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center text-white p-4">
      <div className="w-full max-w-md glass-card p-8 animate-fade-in">
        <h1 className="text-3xl font-bold text-center mb-2 text-transparent bg-clip-text bg-gradient-to-r from-sky-400 to-violet-500">New Monthly Budget</h1>
        <p className="text-center text-slate-300 mb-8">Set your budget for a new month.</p>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="year" className="block text-sm font-medium text-slate-300 mb-2">Year</label>
              <select
                id="year"
                value={year}
            ...
            <div>
              <label htmlFor="month" className="block text-sm font-medium text-slate-300 mb-2">Month</label>
              <select
                id="month"
                value={month}
          ...
          <div>
            <label htmlFor="income" className="block text-sm font-medium text-slate-300 mb-2">Base Monthly Income</label>
            <div className="relative">
              <input
                id="income"
                type="number"

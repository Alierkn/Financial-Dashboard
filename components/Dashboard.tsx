              ...
              <div className={`transition-[max-height,opacity,margin] duration-500 ease-in-out overflow-hidden ${expandedYears[year] ? 'max-h-[1000px] opacity-100 mt-4' : 'max-h-0 opacity-0 mt-0'}`}>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    {groupedData[year].sort((a,b) => b.month - a.month).map((data, idx) => (
                      <div
                        key={data.id}
                        className="relative group glass-card !p-0 overflow-hidden transition-all duration-300 hover:border-slate-600 animate-fade-in"
                        style={{ animationDelay: `${idx * 50}ms` }}
                      >
                        <div
                          onClick={() => onViewMonth(data.id)}
                          role="button"
                          tabIndex={0}
                          aria-label={`View details for ${months[data.month - 1]}`}
